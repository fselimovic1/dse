...
def psat2nd_ukf(settings, ppc, simdata):
    ...
    for i in range(1, tsteps + 1):
        ...
        if i != 1:
            ...
            for j in range(2 * modelorder + 1):
                id = -V * (c1 * math.sin(sigma_xm[0, j] - theta) + c3 * math.cos(sigma_xm[0, j] - theta)) +  c3 * Vf;
                iq = V * (c2 * math.sin(sigma_xm[0, j] - theta) - c1 * math.cos(sigma_xm[0, j] - theta)) + c1 * Vf;
        ...
        for j in range(2 * modelorder + 1):
            ...
            # w function
            id = -V * (c1 * math.sin(sigma_xp[0, j] - theta) + c3 * math.cos(sigma_xp[0, j] - theta)) + c3 * Vf; 
            iq = V * (c2 * math.sin(sigma_xp[0, j] - theta) - c1 * math.cos(sigma_xp[0, j] - theta)) + c1 * Vf;
            sigma_fx[1, j] = sigma_xp[1, j] + tstep/M * (Pm - V * (id * math.sin(sigma_xp[0, j] - theta) + iq * math.cos(sigma_xp[0, j] - theta)) - ra * (id**2 + iq**2) - D*(sigma_xp[1, j] - ws))
